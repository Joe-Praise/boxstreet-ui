...
function MovieCarousel({ doubleMovies }) {
  ...
  const pageCount = Math.ceil(doubleMovies.length / dataPerPage);
  ...
  const displayData = doubleMovies.slice(
    itemsViewed,
    itemsViewed + dataPerPage
  );
  ...
  return (
    <div className="displayMovieContainer">
      ...
      <div className="pagination moblie">
        {activePage >= 4 && <button onClick={() => onChange(0)}>{1}</button>}
        {activePage >= 4 && <span>...</span>}
        {pages.map((page) => (
          <button
